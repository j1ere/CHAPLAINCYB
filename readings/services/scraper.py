...
def fetch_readings(target_date: date = None) -> dict:
    ...
    Args:
        target_date: date object. Defaults to today's date
                     in Africa/Nairobi timezone.
...
def clean_text(raw: str) -> str:
    """
    Clean raw scraped text while preserving meaningful structure.

    Rules:
    - Preserve verse/stanza line breaks
    ...
    for line in lines:
        if line.strip() == "":
            ...
            # Allow max 1 blank separator line
            if blank_run <= 2:
...
def parse_readings(soup: BeautifulSoup, target_date: date) -> dict:
    ...
    for block in soup.find_all("div", class_="innerblock"):
        ...
        # Preserve HTML structure/newlines
        raw_text = (
...
if __name__ == "__main__":
    ...
    # Example custom date:
    # data = fetch_readings(date(2026, 5, 14))
